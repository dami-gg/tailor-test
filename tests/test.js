...
describe('Frontend test', function () {
    ...
    before(() => {
        server = http.createServer(tailorInstance.requestHandler);
        fragment1 = http.createServer(fragmentServer('hello', 'http://localhost:8081'));
        fragment2 = http.createServer(fragmentServer('world', 'http://localhost:8082'));
        return Promise.all([
